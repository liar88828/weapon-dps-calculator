...
export function HomeOld() {
  ...
  const criticalTotalDpsEffective =
    criticalTotalDamagePerMagazine / totalWithSiklus || 0;
  ...
  return (
    <div>
        ...
        <Card>
          ...
          <CardContent className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <MyInput
              ...
              type="number"
              value={fireTime}
              onChange={(v) => setFireTime(Number(v))}
              placeholder="RPM: 600"
            />
            <MyInput
              title="Critical Chance (%)"
              type="number"
              value={criticalChange}
              onChange={(v) => setCriticalChange(Number(v))}
            ...
            <MyInput
              ...
              onChange={(v) => setCriticalMultiplier(Number(v))}
            ...
            <MyInput
              ...
              type="number"
              value={magazine}
              onChange={(v) => setMagazine(Number(v))}
            ...
            <MyInput
              ...
              type="number"
              value={reloadTime}
              onChange={(v) => setReloadTime(Number(v))}
        ...
        <Card>
          ...
          <CardContent className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <p>
                RPS = {fireTime} / 60 = <b>{rps}</b> bullets/sec
              </p>
              <p>
                DPS (no reload) = {damage} × {rps} = <b>{dps}</b>
              </p>
              <p>
                Cycle Time = {magazineRate} + {reloadTime} ={" "}
                <b>{totalWithSiklus}s</b>
              </p>
            ...
            <div>
              ...
              <p>
                Average Damage = <b>{criticalAverage}</b>
              </p>
      ...
      <h2>Normal Damage:</h2>
      <p>
        RPS = {fireTime} / 60 = {rps} peluru/detik
      </p>
      <p>
        DPS non reload = {damage} x {rps} = {dps} DPS
      </p>

      <h3>Normal Damage with Magazine:</h3>
      <p>
        Waktu habis mag = {magazine} / {rps} = {magazineRate} detik
      </p>
      <p>
        Total waktu siklus = {dps} x {magazineRate}= {totalWithSiklus} detik
      </p>
      <p>
        Total damage per mag = {dps} x {magazine}= {totalDamagePerMagazine}{" "}
      </p>
      <p>
        DPS efektif = {totalDamagePerMagazine} / {totalWithSiklus} ≈{" "}
        {totalDpsEffective} DPS
      </p>

      <h2>With Critical Damage:</h2>
      <p className={"text-nowrap"}>
        Damage Critical = ({damage} x {(100 - criticalChange) / 100}) + (
        {damage} x {criticalMultiplier} x {criticalChange / 100}) ={" "}
        {criticalAverage}
      </p>
      <p>
        DPS Critical = {criticalAverage} x {rps} = {criticalDps} DPS
      </p>

      <h3>Normal Damage with Magazine:</h3>
      <p>
        Total damage per mag = {criticalDps} x {magazine}={" "}
        {criticalTotalDamagePerMagazine}{" "}
      </p>
      <p>
        Total DPS efektif = {criticalTotalDamagePerMagazine} / {totalWithSiklus}{" "}
        ≈ {criticalTotalDpsEffective} DPS
      </p>
    </div>
  );
}
